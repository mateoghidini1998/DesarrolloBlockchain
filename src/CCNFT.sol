...
// Constructor (nombre y símbolo del NFT).    
    constructor() ERC721("CryptoCampo NFT", "CCNFT") {
        // Valores por defecto; configurables por setters
        maxBatchCount = 20;
        buyFee = 0;
        tradeFee = 0;
        profitToPay = 0;
...
// Parametro amount: La cantidad de NFTs que se quieren comprar.
    function buy(uint256 value, uint256 amount) external nonReentrant {
        require(canBuy, "Buy disabled"); // Verificación de permisos de la compra con "canBuy". Incluir un mensaje de falla.
...
// Verificacón del valor total después de la compra (no debe exeder el valor máximo permitido "maxValueToRaise"). Incluir un mensaje de falla.
        require(totalValue + value * amount <= maxValueToRaise, "Cap exceeded");
        ...
        for (uint256 i = 0; i < amount; i++) { // Bucle desde 1 hasta amount (inclusive) para mintear la cantidad especificada de NFTs.
            ...
            _safeMint(_msgSender(), tokenId); // Minteo de NFT y asignación al msg.sender.
            emit Buy(_msgSender(), tokenId, value); // Evento Buy con el comprador, el tokenId y el valor del NFT.
...
// Parámetros: Lista de IDs de tokens de reclamo (utilizar calldata).
    function claim(uint256[] calldata listTokenId) external nonReentrant {
        ...
        require(canClaim, "Claim disabled"); // Verificacón habilitación de "reclamo" (canClaim). Incluir un mensaje de falla.

        require(listTokenId.length > 0 && listTokenId.length <= maxBatchCount, "Invalid list length"); // Verificacón de la cantidad de tokens a reclamar (mayor que 0 y menor o igual a maxBatchCount). Incluir un mensaje de falla.
        ...
        for (uint256 i = 0; i < listTokenId.length; i++) { // Bucle para iterar a través de cada token ID en listTokenId.
			...
			require(_exists(listTokenId[i]), "Token !exists"); // Verificacón listTokenId[i] exista. Incluir un mensaje de falla.
...
// Transferir los fondos desde fundsCollector al (_msgSender()).
        ...
        if (!fundsToken.transferFrom(fundsCollector, _msgSender(), payout)) {
...
// Funcion de compra de NFT que esta en venta.
    function trade(uint256 tokenId) external nonReentrant { // Parámetro: ID del token.
        require(canTrade, "Trade disabled"); // Verificación del comercio de NFTs (canTrade). Incluir un mensaje de falla.
        require(_exists(tokenId), "Token !exists"); // Verificación de existencia del tokenId (_exists). Incluir un mensaje de falla.
// Verificamos que el comprador (el que llama a la función) no sea el propietario actual del NFT. Si lo es, la transacción falla con el mensaje "Buyer is the Seller".
        ...
        require(seller != _msgSender(), "Buyer is the Seller"); // Verificación de propietario actual del NFT no sea el comprador. Incluir un mensaje de falla.
...
// Transferencia del precio de venta del comprador al propietario actual del NFT usando fundsToken.
        if (!fundsToken.transferFrom(_msgSender(), seller, tokenSale.price)) {
            revert("Cannot send price"); // Incluir un mensaje de falla.
        }
...
// Transferencia de tarifa de comercio (calculada como un porcentaje del valor del NFT) del comprador al feesCollector.
       if (!fundsToken.transferFrom(_msgSender(), feesCollector, values[tokenId] * tradeFee / 10000)) {
            revert("Cannot send trade fee"); // Incluir un mensaje de falla.
        }
  
        emit Trade(); // Registro de dirección del comprador, dirección del vendedor, tokenId, y precio de venta.  

        _safeTransfer(seller, _msgSender(), tokenId, ""); // Transferencia del NFT del propietario actual al comprador.
...
// Función para poner en venta un NFT.
    function putOnSale(uint256 tokenId, uint256 price) external { // Parámetros: ID y precio del token.
        require(canTrade, "Trade disabled"); // Verificación de operaciones de comercio (canTrade). Incluir un mensaje de falla.

        require(_exists(tokenId), "Token !exists"); // Verificción de existencia del tokenId mediante "_exists". Incluir un mensaje de falla.

        require(ownerOf(tokenId) == _msgSender(), "Only owner"); // Verificción remitente de la transacción es propietario del token. Incluir un mensaje de falla.
...
// Utilización del token ERC20 para transacciones.
    function setFundsToken(address token) external onlyOwner { // Parámetro, token: Que va a ser la Dirección del contrato del token ERC20.                                                      
        require(token != address(0), "Invalid token"); // La dirección no puede ser la dirección cero (address(0)). Incluir un mensaje de falla.
